...
gemco_customer = spark.read.parquet("gs://prd-lake-raw-atena/gemco_cliente_full/").select([col(x).alias(renamed_gemco[original_gemco.index(x)]) for x in original_gemco]).withColumn("peso_base_origem", ranking)
...
# READING ONLINE
online_customer = spark.read.parquet("gs://prd-lake-raw-atena/online_cliente/").select([col(x).alias(new_online[original_online.index(x)]) for x in original_online]).withColumn("peso_base", ranking)
...
final = grouped.join(zipc, cond, "left_outer").drop("cep", "etl_id")                               .withColumn("cep_validation", get_tpvalidation("tipo_cep", "numero_res"))                               .withColumn("flvalidcep", when(col("cep_validation") != 0, 1).otherwise(lit(0)))                               .withColumn("endereco_compl_tratado", concat(col("codlograd_cep"), lit(" "), 
                                col("numero_res"), lit(" "), col("complemento_res")))\
                               .withColumn("cep_tratado", col("cep_res"))\
                               .withColumn("codlograd_tratado", when(col("cep_validation") == 2, col("codlograd_cep")).otherwise(col("codlograd_res")) )\
                               .withColumn("lograd_tratado", when(col("cep_validation") == 2, col("lograd_cep")).otherwise(col("lograd_res")))\
                               .withColumn("endereco_compl_tratado", when(col("cep_validation") == 2, col("endereco_compl_tratado")).otherwise(col("endereco_completo")))\
                               .withColumn("endereco_tratado", when(col("cep_validation") == 2, col("endereco_cep")).otherwise(col("endereco_res")))\
                               ...
                               .withColumn("estado_tratado", when(col("cep_validation") == 0, col("estado_res")).otherwise(get_state(col("uf_cep"))))\
