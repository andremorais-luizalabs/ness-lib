...
trimmed_df = df.select([remove_accents_udf(col(col_name)).alias(col_name) for col_name in df.columns])               .select([trim(col(col_name)).alias(col_name) for col_name in df.columns])               .select([col(x).alias(new_columns[y]) for y,x in enumerate(original_columns)])               .selectExpr(select_expr_list + list_transform).filter(col("id_cliente").isNotNull()) 


# In[13]:


transform = trimmed_df.withColumn("flvalidemail", valid_email_udf(col("email_cliente")))                      .withColumn("natureza_juridica", when(col("ind_pessoa_fisica_cliente") == False, lit("J")).otherwise(lit("F")))                      .withColumn("dt_cadastro_cliente", date_format("dt_cadastro_cliente", "yyyy-mm-dd"))                      .withColumn("estado_civil", estado_civil_udf(col("estado_civil")))                      .withColumn("origem", lit("ECOMM"))                      .withColumn("valid_cgccpf", valid_cgccpf("natureza_juridica", "cpf_cnpj_cliente"))                      .withColumn("dddcel", get_ddd_from_fullphone("celphone"))                      .withColumn("dddphone", get_ddd_from_fullphone("celphone"))                      .withColumn("codlograd_cliente", lit("NAO INFORMADO"))                      .withColumn("lograd_cliente", lit("NAO INFORMADO"))                      .withColumn("endereco_completo", concat(col("endereco_cliente"), lit(" "),
                                                              col("numero_end_cliente"), lit(" "), col("complemento_end_cliente")))
...
squery = transform.writeStream                   .partitionBy("etl_id")                   .format("parquet")                   .trigger(once=True)                   .option("checkpointLocation", "gs://prd-lake-transient-atena/checkpoints/crm_online_cliente/")                   .option("path", "gs://prd-lake-raw-atena/online_cliente/")                   .start()                   .awaitTermination()
