...
destination_customer = "gs://prd-lake-raw-atena/gemco_cliente/"
custom_squery = customer.writeStream                         .partitionBy("etl_id")                         .format("parquet")                         .trigger(once=True)                         .option("checkpointLocation", "gs://prd-lake-transient-atena/checkpoints/crm_gemco_cliente/")                         .option("path", destination_customer)                         .start() 
...
address_treated = addr_df.select([default_for_null(remove_accents_udf(upper(trim(col(col_name))))).alias(col_name)                         for col_name in addr_df.columns]) 
...
destination_addrs = "gs://prd-lake-raw-atena/gemco_cliente_addr/"
add_custom_squery = address_renamed.writeStream                         .partitionBy("etl_id")                         .format("parquet")                         .trigger(once=True)                         .option("checkpointLocation", "gs://prd-lake-transient-atena/checkpoints/crm_gemco_cliente_addr/")                         .option("path", destination_addrs)                         .start()                         .awaitTermination()
...
destination_card = "gs://prd-lake-raw-atena/gemco_cliente_card/"
card_custom_squery = card.writeStream .partitionBy("etl_id") .format("parquet") .trigger(once=True) .option("checkpointLocation", "gs://prd-lake-transient-atena/checkpoints/crm_gemco_cliente_card/") .option("path", destination_card) .start() .awaitTermination()
...
addr_commercial = raw_addr.filter("tpender == 'M'").select([(col(col_name)).alias(col_name+"_com")                         for col_name in raw_addr.columns]).withColumnRenamed("codcli_com", "codcli")


# In[20]:


joined_df = raw_customer.join(addr_residential, 'codcli', 'left_outer').join(addr_commercial, 'codcli', 'left_outer').join(raw_card, 'codcli', 'left_outer')
...
squery = customer_full.write.partitionBy("etl_id").mode("overwrite").parquet("gs://prd-lake-raw-atena/gemco_cliente_full/")
